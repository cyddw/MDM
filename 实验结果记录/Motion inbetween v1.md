...
<summary>📖 实验记录</summary>
    ...
    1.将MMFi数据集在Ours模型中的关节链中的shoulder和wrist交换，其他不变，重新训练，观察是否会出现nan(已完成，没有出现nan)
...
<summary>📖 实验记录</summary>
    ...
    1.将MMFi数据集在Ours模型中的segment(20-10)改为segment(20-20)(已完成，模型的性能有提升(MPJPE = 400->264))
    2.将MMFi数据集在Ours模型中的segment(20-10)改为segment(10-5)(已完成，模型的性能有提升(MPJPE = 235))
</details>  
...
<summary>📖 问题记录</summary>  
    ...
    1.已找到导致Person in wifi和MetaFi模型报错的原因：mesh数据中的005570.npy文件有误，是空的
</details>  

<details>
<summary>📖 实验记录</summary>

    1.将MMFi数据集在Ours模型上进行训练(10-5)，finetune(lr = 0.1lr)(已完成，模型性能有提升)
    2.将MDM数据集在MetaFi模型上进行训练(已完成)
    3.将MDM数据集在Person in wifi数据集上进行训练(已完成)
</details>  
</details>

...
1.从关节空间坐标转为旋转表示的过程：  
  ...
  1.2通过逆运动学算法将关节坐标位置转为旋转参数(四元数):通过肩关节等计算得到前向方向向量；计算从前向方向向量旋转为目标方向向量[0,0,1]的四元数  
  1.3
</details>  
...
2.训练nsub = 30，segment = 20的frozen(预训练 epoch = 200，segment = 20)  
3.训练nsub = 30，segment = 20的finetune(预训练 epoch = 200，segment = 20, lr = 0.01LR)
</details>  
